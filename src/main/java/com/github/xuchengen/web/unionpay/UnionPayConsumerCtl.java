 ...
 */
@Controller
@RequestMapping(value = "/unionPay/onlineGatewayPay/consumer")
public class UnionPayConsumerCtl extends BaseCtl {

    private static final Log log = LogFactory.get(UnionPayConsumerCtl.class);

    @GetMapping(value = {"", "/"})
    public String index(ModelMap modelMap) {
        modelMap.put("returnUrl", getHostUrl() + "/unionPay/onlineGatewayPay/consumer/doReturn");
        modelMap.put("notifyUrl", getHostUrl() + "/unionPay/doNotify");
        return "/unionpay/onlineGateway/consumer.html";
    }
    ...
    public String doPay(String orderNo,
        ...
        try {
            ...
            log.error("调用银联在线网关支付消费接口异常：{}", JSONUtil.toJsonStr(e));
            throw new RuntimeException("银联在线网关支付消费接口异常", e);
        }
    }
    ...
    public String doReturn() {
        ...
        if (verify) {
            ...
            log.debug("接收银联同步回调通知：消费接口[验签失败]");
        }

        return "redirect:/unionPay/onlineGatewayPay/consumer";
    }
