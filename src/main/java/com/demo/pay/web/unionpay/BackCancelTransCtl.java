...
import com.demo.pay.unionpay.request.UnionPayBackTransRequest;
 ...
 */
@Controller
@RequestMapping(value = "/unionpay/backCancelTrans")
public class BackCancelTransCtl extends BaseCtl {

    private static final Log log = LogFactory.get(BackCancelTransCtl.class);
    ...
    public ResponseWrapper index() {
        try {
            ...
            unionPayBackTransRequest.setOrderId(OrderNoUtil.generate());
            unionPayBackTransRequest.setOrigQryId("451908301547274711258");
            unionPayBackTransRequest.setTxnAmt("100");
            unionPayBackTransRequest.setTxnType("31");
            ...
            unionPayBackTransRequest.setBackUrl(getHostUrl() + "/unionpay/backCancelTrans/notify");
    ...
    public String doNotify() {
        ...
        if (verify) {
            log.debug("接收银联异步回调通知：消费撤销接口[验签成功]");
            return "OK";
        }
