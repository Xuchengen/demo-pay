 ...
 */
@Controller
@RequestMapping(value = "/unionpay/preAuthFinish")
public class PreAuthFinishCtl extends BaseCtl {

    private static final Log log = LogFactory.get(PreAuthFinishCtl.class);
    ...
    public ResponseWrapper index() {
        try {
            ...
            unionPayPreAuthFinishRequest.setOrigQryId("731908301722284148768");
            unionPayPreAuthFinishRequest.setTxnAmt("100");
            unionPayPreAuthFinishRequest.setTxnType("03");
            ...
            unionPayPreAuthFinishRequest.setBackUrl(getHostUrl() + "/unionpay/preAuthFinish/notify");
    ...
    public String asyncNotify() {
        ...
        if (verify) {
            log.debug("接收银联异步回调通知：预授权完成接口[验签成功]");
            return "OK";
        }
