 ...
 */
@Controller
@RequestMapping(value = "/unionpay/preAuth")
public class PreAuthCtl extends BaseCtl {

    private static final Log log = LogFactory.get(PreAuthCtl.class);
    ...
    public String index() {
        try {
            ...
            unionpayFrontTransRequest.setFrontUrl(getHostUrl() + "/unionpay/preAuth/return");
            unionpayFrontTransRequest.setBackUrl(getHostUrl() + "/unionpay/preAuth/notify");
    ...
    public String asyncNotify() {
        ...
        if (verify) {
            log.debug("接收银联异步回调通知：预授权接口[验签成功]");
            return "OK";
        }
