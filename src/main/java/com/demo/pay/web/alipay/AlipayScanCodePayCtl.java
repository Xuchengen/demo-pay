...
/**
 * 支付宝-扫码支付
 * 用户扫描商户二维码进行支付，商户服务器端须接收支付宝回调进行后续处理
 */
@Controller
...
public class AlipayScanCodePayCtl extends BaseCtl {
    ...
    private static final String APP_ID_CACHE = "alipay:scan_code_pay:app_id";
    private static final String CHARSET_CACHE = "alipay:scan_code_pay:charset";
    private static final String SIGN_TYPE_CACHE = "alipay:scan_code_pay:sign_type";
    private static final String MERCHANT_PRIVATE_KEY_CACHE = "alipay:scan_code_pay:merchant_private_key";
    private static final String ALIPAY_PUBLIC_KEY_CACHE = "alipay:scan_code_pay:alipay_public_key";
    ...
    public String page(ModelMap modelMap) {
        modelMap.put("notifyUrl", getHostUrl() + "/alipay/scanCodePay/notify");
     ...
     * 生成收款二维码
     */
    @PostMapping(value = "/doPay", produces = MediaType.IMAGE_PNG_VALUE)
    public ResponseEntity<byte[]> doPay(String server_url,
                                        String app_id,
        ...
        try {
            ...
            alipayTradePrecreateRequest.setApiVersion(version);
            alipayTradePrecreateRequest.setNotifyUrl(notify_url);

            AlipayTradePrecreateModel alipayTradePrecreateModel = new AlipayTradePrecreateModel();
            alipayTradePrecreateModel.setOutTradeNo(out_trade_no);
            alipayTradePrecreateModel.setSubject(subject);
            alipayTradePrecreateModel.setTotalAmount(total_amount);
            ...
            if (alipayTradePrecreateResponse.isSuccess()) {
                byte[] bytes = QrCodeUtil.generatePng(alipayTradePrecreateResponse.getQrCode(), 300, 300);
                HttpHeaders httpHeaders = new HttpHeaders();
                httpHeaders.setContentType(MediaType.IMAGE_PNG);
                return new ResponseEntity<>(bytes, httpHeaders, HttpStatus.OK);
            } else {
                log.warn("调用支付宝扫码支付接口失败：{}", JSONUtil.toJsonStr(alipayTradePrecreateResponse));
                getResponse().sendRedirect("/alipay/scanCodePay");
                return null;
            }
        } catch (Exception e) {
